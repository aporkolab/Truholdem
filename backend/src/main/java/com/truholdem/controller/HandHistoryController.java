...
@Tag(name = "Hand History", description = "Hand history and replay operations")
@CrossOrigin(origins = "*")
public class HandHistoryController {
    ...
    @Operation(summary = "Get a specific hand history")
    public ResponseEntity<HandHistory> getHandHistory(@PathVariable UUID historyId) {
    ...
    @Operation(summary = "Get all hands for a game")
    public ResponseEntity<List<HandHistory>> getGameHistory(@PathVariable UUID gameId) {
        List<HandHistory> history = handHistoryService.getGameHistory(gameId);
        return ResponseEntity.ok(history);
    }

    @GetMapping("/game/{gameId}/paged")
    @Operation(summary = "Get hands for a game with pagination")
    ...
    @Operation(summary = "Get hands where a player participated")
    public ResponseEntity<List<HandHistory>> getPlayerHistory(@PathVariable UUID playerId) {
        List<HandHistory> history = handHistoryService.getPlayerHistory(playerId);
        return ResponseEntity.ok(history);
    }

    @GetMapping("/wins/{playerName}")
    @Operation(summary = "Get hands where a player won")
    public ResponseEntity<List<HandHistory>> getPlayerWins(@PathVariable String playerName) {
        List<HandHistory> history = handHistoryService.getPlayerWins(playerName);
        return ResponseEntity.ok(history);
    }

    @GetMapping("/recent")
    @Operation(summary = "Get recent hands")
    ...
    @Operation(summary = "Get hands with biggest pots")
    ...
    @Operation(summary = "Get replay data for a hand")
    public ResponseEntity<HandHistoryService.ReplayData> getReplayData(@PathVariable UUID historyId) {
    ...
    @Operation(summary = "Get hand count for a game")
    public ResponseEntity<Long> getHandCount(@PathVariable UUID gameId) {
        long count = handHistoryService.getHandCount(gameId);
        return ResponseEntity.ok(count);
    }

    @DeleteMapping("/game/{gameId}")
    @Operation(summary = "Delete all history for a game")
    public ResponseEntity<Void> deleteGameHistory(@PathVariable UUID gameId) {
