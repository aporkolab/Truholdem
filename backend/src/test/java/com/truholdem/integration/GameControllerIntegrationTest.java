...
class GameControllerIntegrationTest {
                ...
                void shouldAdvanceToNextPhase() throws Exception {
                        ...
                        for (int i = 0; i < 10; i++) {
                                ...
                                if (!current.isFolded() && !current.isAllIn()) {
                                        Map<String, Object> action = Map.of("action", "CALL", "amount", 0);
                                        mockMvc.perform(post(BASE_URL + "/{g}/player/{p}/action",
                        ...
                        assertThat(finalGame.getPhase())
                                        .isIn(GamePhase.FLOP, GamePhase.TURN, GamePhase.RIVER, GamePhase.SHOWDOWN);
                }
        }
        ...
        class FullGameFlowTests {
                ...
                void shouldCompleteFullGameWithShowdown() throws Exception {
                        ...
                        MvcResult createResult = mockMvc.perform(post(BASE_URL + "/start")
                                        .contentType(MediaType.APPLICATION_JSON)
                        ...
                        int max = 20;
                        while (!game.isFinished() && max-- > 0) {
                                Player current = game.getPlayers().get(game.getCurrentPlayerIndex());

                                Map<String, Object> action = Map.of("action", "CALL", "amount", 0);

                                MvcResult actionResult = mockMvc.perform(post(BASE_URL + "/{g}/player/{p}/action",
                                                game.getId(), current.getId())
                                                .contentType(MediaType.APPLICATION_JSON)
                                                .content(objectMapper.writeValueAsString(action)))
                                                .andExpect(result -> {
                                ...
                                if (actionResult.getResponse().getStatus() == 200) {
